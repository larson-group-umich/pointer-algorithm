...
MODULE SHARED
  ...
  REAL :: CLIST(10500,400),CONST,CREP,CFL,T(5000),G(5000),GF(5000)
  REAL :: tMIN,tMAX,TAGET_MIU(40),TAGET_TAO(40),PC,PM,Ls,d,MERR(2)
  ...
  REAL :: ZEMA,G0MA,G0MI,Kb,Vs,TEM,PHI,PI,RELAX,GERR(3),t0,VERR
  REAL :: V0,RANG,INDEX,BDIND,DT,TRY_ERRG,TRY_MIU(40),TRY_TAO(40)

  INTEGER :: LMAP(10500),LBREATH,SLP,NUM,NUMG,NUMIN,FLAGPR,FLAGD
  INTEGER :: NN,SAMPLE,GENERATION,NSEED,SERR(2),FLAGMR,II,FLAGIT
  INTEGER :: NIND,TRY_NN,FLAGN,FLAGFC2,FLAGMGT,Nt0 
...
PROGRAM MAIN
  ...
  REAL :: RATIOF,LpF,ZEF,APH,RATIO,Lp,ZE,FMI,FMA,PERR(6),FERR(5),G0F,WB
  REAL :: MLIST1(400),MLIST2(400),MLIST3(400),GGG(5000),GG(5000),DW
  REAL :: REMAIN,BR,RND,G1,G2,ERRG,GGS,GGGS,GGC,GGGC,D0,W,La,MARK,EAPH
  REAL :: tbr,trep,te,Le,G0,ZETA,FIC,FIC2,TL,tp,LBAR,ST,OS,APHU,APHL
  REAL :: FIC3,tR,FIC4,CLENTH,G0R,Dr,VOF,LENGTH(5050),Li,APH0,RATIO0
  REAL :: G21,G22,C1,C2,XCHR,DGMAX,APH1,GRATIO,FRATIO

  INTEGER :: I,J,JJ,JJJ,K,KK,KKK,TT,N,M,LSEGL,LENTH,SUM1,SUM2,INV,REM
  INTEGER :: FLAGC2,FLAGH,FLAGF,FLAGIM,FLAGG,IDC1,IDM,LDR,LSEG,FLAGSS
  INTEGER :: LROUSE,LBEND,NUMOU,OUTF,OUTW,OUTU,IDATE(8),NLFU,NLFL
  ...
  APHMA=10.		 ! THE UPPER BOUNDARY OF Le/Lp
  ...
  FLAGSS=0		 ! FLAG FOR STARTING SIMULATION FROM SPECIFIED PARAMETER VALUES
  ...
  OPEN(16,FILE='INPUT_v3_1.DAT')       ! EXPERIMENTAL DATA INPUT FILE
  READ(16,*) TITLE
  READ(16,*) TEM,PHI,Vs
...
10 CLOSE(16)
  ...
  IF (FLAGF.EQ.0)	THEN
     WRITE(24,*) 'ERROR: FREQUENCY INFORMATION DOES NOT EXSIST'
     WRITE(*,*) 'ERROR: FREQUENCY INFORMATION DOES NOT EXSIST'
     STOP
  END IF
  IF (FLAGMR.EQ.1) THEN
  ...
  DGMAX=(GGE(1)-GGGE(1))/GGE(1)
  DO J=1,NUMIN
     IF ((GGE(J)-GGGE(J))/GGE(J).GT.DGMAX) THEN
	DGMAX=(GGE(J)-GGGE(J))/GGE(J)
     END IF
  END DO

  IF (DGMAX.LT.0.5) THEN
     FLAGMGT=1
  END IF
  ...
  FRATIO=10.
  DO J=IDC1,NUMIN-1
     IF ((GGGE(J-1).GE.GGGE(J)).AND.(GGGE(J+1).GE.GGGE(J))) THEN
	FRATIO=FQE(J)/FQC1
	EXIT
     END IF
  END DO
...
33            CALL RANDOM_NUMBER(RND)
	...
	CLOSE(7)
     ...
     t0=0.
     DO I=1,NN
	t0=t0+EXP(TAGET_MIU(I))*TAGET_TAO(I)
     END DO
     VERR=LOG(t0*G0/V0)
...
SUBROUTINE GEAM(FLAGi5,No5,Mo5,To5,ERRMo5)
  ...
  REAL :: RNDs5,MIU(505,40),IND1s5,IND2s5,TAO(505,40),OPT,Ds5,ERRSD(20)
  REAL :: RSUMs5,ERR(505),MINs5,R1s5,R2s5,CRs5,RA1s5,RA2s5,BDRs5(2,40)
  REAL :: XCHMs5(2,2,40),SEEDs5(20,2,40),OPT0,CNSTR1s5,CNSTR2s5
  REAL, INTENT(OUT) :: Mo5(40),To5(40),ERRMo5
        ...
        S12:DO WHILE ((CHECK+RESCALE1+RESCALE2).NE.0)
           ...
           DO Js5=1,NMOD(Is5)
              IF (MIU(Is5,Js5)-T(1)/TAO(Is5,Js5).GT.-20.) THEN
                 RSUMs5=RSUMs5+EXP(MIU(Is5,Js5)-T(1)/TAO(Is5,Js5))
              END IF
           END DO

           IF (ABS(RSUMs5-G(1))/G(1).LT.CNSTR1s5) THEN
              RESCALE1=0
           ELSE
              RESCALE2=1
              R1s5=LOG(G(1)/RSUMs5)
              DO Js5=1,NMOD(Is5)
                 ...
                 MIU(Is5,Js5)=MIU(Is5,Js5)+R1s5-R2s5
           ...
           IF (ABS(RSUMs5-G(NUMG))/G(NUMG).LT.CNSTR2s5) THEN
              RESCALE2=0	
           ...
           ELSE
              ...
              DO Js5=1,NMOD(Is5)
                 IF (MIU(Is5,Js5)-T(NUMG)/TAO(Is5,Js5).GT.-20.) THEN
                    IND1s5=RA1s5-MIU(Is5,Js5)
                    IND2s5=RA2s5-MIU(Is5,Js5)
                    CALL RANDOM_NUMBER(RNDs5)
...
SUBROUTINE PRDF(G0i7,RATIOi7,ZEi7,Lpi7,G0o7,RATIOo7,ZEo7,Lpo7,FLAGo7)
  ...
  REAL :: C1s7,C2s7,APHo7,APHi7,APH1s7
  ...
  ELSE
     ZEo7=ZEi7*(FQM/FQFM*FQFC1/FQC1)**RELAX
  END IF
  IF (ABS(GERR(1)).LT.0.1) THEN
     IF (FLAGMGT.EQ.0) THEN
	ZEo7=ZEo7**RELAX*(FQM/FQFM*FQFC1/FQC1)**(1.-RELAX)
     ELSE
 	ZEo7=ZEo7**RELAX*(GGM/GGFM*GGGFM/GGGM)**(1.-RELAX)
     END IF
  END IF
  ...
  IF (ABS(LOG(GGGC1/GGGFC1)).LT.0.1) THEN
  ...
  IF (FLAGo7.EQ.0) THEN
	...
	RATIOo7=RATIOi7*(FQFC1/FQC1*(Lpi7*ZEi7/Lpo7/ZEo7)**3.)**(1.5*RELAX)
     ELSE
        RATIOo7=RATIOi7*(FQFC1/FQC1*(Lpi7*ZEi7/Lpo7/ZEo7)**3.*APHo7/APHi7)**(2.*RELAX)
     END IF
