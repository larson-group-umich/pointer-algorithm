...
MODULE SHARED
  ...
  INTEGER :: NN,SAMPLE,GENERATION,NELT,INDEX,TAGET_B(40),SERR(2)
  INTEGER :: FLAGMR,II,FLAGIT,FLAGN 
...
PROGRAM MAIN
  ...
  REAL :: MLIST1(400),MLIST2(400),MLIST3(400),GGG(5000),GG(5000),DW,DT
  ...
  INTEGER :: LBR,NEWL1,NEWL2,NEWL3,LPI(3),XCH,NM,Z,NLF,MARK,ITER
  INTEGER :: NLFL
  ...
  APHMA=6.			         ! THE UPPER BOUNDARY OF Le/Lp
  APHMI=0.5			         ! THE LOWER BOUNDARY OF Le/Lp
  RATIOMA=25.			         ! THE UPPER BOUNDARY OF BREAKAGE RATE (ZETA)
  RATIOMI=10.**(-3.)                     ! THE LOWER BOUNDARY OF BREAKAGE RATE (ZETA)
  LpMA=1.5*10**(-7.)		         ! THE UPPER BOUNDARY OF Lp
  LpMI=1.5*10**(-8.)		         ! THE LOWER BOUNDARY OF Lp
  ZEMA=100.				 ! THE UPPER BOUNDARY OF Z
  ...
  SLP=0					 ! THE SWITCHER FOR MODIFYING lp DURING ITERATIONS
  ...
  OPEN(16,FILE='INPUT.DAT')	! EXPERIMENTAL DATA INPUT FILE
  READ(16,*) TITLE
  READ(16,*) TEM,PHI,Vs
...
10 CLOSE(16)
  ...
  IF (FLAGF.EQ.0) THEN
     WRITE(24,*) 'ERROR: FREQUENCY INFORMATION DOES NOT EXIST'
     WRITE(*,*) 'ERROR: FREQUENCY INFORMATION DOES NOT EXIST'
     STOP
  END IF
  IF (FLAGMR.EQ.1) THEN
  ...
  GGC1=GGE(1)		    ! EXPERIMENTAL VALUE OF G' AT 1st CROSSOVER  
  GGGC1=GGGE(1)		    ! EXPERIMENTAL VALUE OF G" AT 1st CROSSOVER
  GGM=GGE(NUMIN)            ! EXPERIMENTAL VALUE OF G' AT INTERMEDIATE FREQUENCY 
  GGGM=GGGE(NUMIN)	    ! EXPERIMENTAL VALUE OF G" AT INTERMEDIATE FREQUENCY
  ...
  END IF
  IF (FLAGH.EQ.0) THEN
     WRITE(24,*) 'WARNING: EXPERIMENT DOES NOT REACH HIGH FREQUENCY ZONE'
     WRITE(24,*) 'INITIAL GUESS USED FOR PERSISTENCE LENGTH'
     WRITE(*,*) 'WARNING: EXPERIMENT DOES NOT REACH HIGH FREQUENCY ZONE'
  ...
  G0MA=GGM*3.		             ! THE UPPER BOUNDARY OF PLATEAU MODULUS
  ...
  Z01:DO WHILE ((EAPH.GT.10**3.).OR.(K.LT.10))
	...
	TL=1.
     ...
     ZE=Li/APH/Lp				 ! NUMBER OF ENTANGLEMENTS
  ...
  Z00:DO WHILE (JJ.LE.ITER)	! ITERATION STARTS				 			
     FLAGPR=0	                ! FLAG FOR OVER-WRITING PARAMETERS & OUTPUT FILE			        
     ...
     !==================================================================
     !	   LOW FREQUENCY SIMULATION (REPTATION, BREAKAGE+REJOINING)
     !==================================================================
...
21   WRITE(24,*) 'LENGTH DISCRETIZATION FINISHED'
     WRITE(*,*) 'LENGTH DISCRETIZATION FINISHED'

     WRITE(24,*) 'THE MINIMUM LENGTH IS',Lp*Ls
     WRITE(24,*) 'THE MAXIMUM LENGTH IS',(1+(M-1)*N)*Ls*Lp
     WRITE(*,*) 'THE MINIMUM LENGTH IS',Lp*Ls
     WRITE(*,*) 'THE MAXIMUM LENGTH IS',(1+(M-1)*N)*Ls*Lp

     SUM2=0.
     LSEG=0
     DO I=1,M
        LENGTH(I)=CEILING(NUM*N*Ls*Lp*EXP(-1.*(1+(I-1)*N)*Ls*Lp/La)/La)
        IF ((I.EQ.M).AND.(SUM2+LENGTH(I).GE.NUM)) THEN
           LENGTH(I)=NUM-SUM2
        END IF
        SUM1=1+SUM2
        SUM2=SUM2+LENGTH(I)
        LSEG=LSEG+(1+(I-1)*N)*LENGTH(I)		
        DO J=SUM1,SUM2
           LMAP(J)=1+(I-1)*N	
           CLIST(J,1)=1.
           CLIST(J,2)=0.
        ...
        END DO
     END DO
     WRITE(24,*) 'CHAIN LIST INITIATION FINISHED'
     WRITE(*,*) 'CHAIN LIST INITIATION FINISHED'
...
33            CALL RANDOM_NUMBER(RND)
              ...
              IF (LPI(2).EQ.LPI(1)) THEN
                 GOTO 32
              END IF
              IF ((LMAP(LPI(1))+LMAP(LPI(2))).GT.M) THEN
                 GOTO 32
              END IF
        ...
        IF (KKK.LE.100) THEN			! WRITE TIME DOMAIN DATA IN THE TEMPORARY FILE
           WRITE(21,*)	K,K*DT,G2
           KKK=KKK+1
        ELSE
           IF ((KKK.LE.300).AND.(MOD(K,5).EQ.0)) THEN
              WRITE(21,*) K,K*DT,G2
              KKK=KKK+1
           ELSE
              IF ((KKK.LE.700).AND.(MOD(K,20).EQ.0)) THEN
                 WRITE(21,*) K,K*DT,G2
                 KKK=KKK+1
              ELSE
                 IF ((KKK.LE.1500).AND.(MOD(K,100).EQ.0)) THEN
                    WRITE(21,*) K,K*DT,G2
                    KKK=KKK+1
                 ELSE
     ...
     DO I=4,9
        IF (REM/10.**I.LT.1.) THEN 
           ...
           REM=XCH*10.**(I-1.)+CEILING((REM/10.**(I-2.)-XCH*10.))*10.**(I-2.)
           GOTO 41
        END IF
     END DO
...
41   WRITE(*,*) 'REM',REM
     ...
     INDEX=(REM/10000.)**0.25
     CALL GGT(INDEX)		  ! PICK APPROPRIATE NUMBER OF THE DATA FROM TEMPORARY FILE

     WRITE(*,*) 'INITIATION OF GA FINISHED'
     WRITE(24,*) 'INITIATION OF GA FINISHED'

     !==================================================================
     !  	TIME TO FREQUENCY TRANSFORMATION (GENETIC ALGORITHM)
     !==================================================================
     ...
     WRITE(24,*) 'RELAXATION TIME CALCULATION FINISHED'
     WRITE(*,*) 'RELAXATION TIME CALCULATION FINISHED'

     IF (OUTF.EQ.1) THEN
        OPEN(7,FILE='GF_t.DAT')	  ! WRITE OUTPUT FILE FOR FITTED G(t) GENERATED BY GA
     ...
     !==================================================================
     !           HIGH FREQUENCY SIMULATION (BENDING+ROUSE MODES)
     !==================================================================
     ...
     A02:IF (LROUSE.EQ.1) THEN  ! ROUSE MODES
        DO I=1,NUMOU
           ...
           DO J=1,M
              Z=CEILING((1+(J-1)*N)*Ls*TL/APH)
              NM=CEILING((1+(J-1)*N)*Ls*TL/2.)
              IF (Z.GE.1) THEN
                 VOF=LENGTH(J)/NUM*(1+(J-1)*N)*Ls/LBAR
              ELSE
                 CYCLE
              END IF
              GGC=0.
        ...
        WRITE(24,*) 'ROUSE MODES INCLUDED'
        WRITE(*,*) 'ROUSE MODES INCLUDED'
     END IF A02
     ...
     IF (LBEND.EQ.1) THEN 	! BENDING MODES
        DO I=1,NUMOU
           ...
           GG(I)=GG(I)*G0
           GGG(I)=GGG(I)*G0
           GG(I)=GG(I)+(ST*W**0.75)*WB
           GGG(I)=GGG(I)+(OS*W**0.75+W*Vs)*WB
        END DO
        ...
        DO I=1,NUMIN
           ...
           GGF(I)=GGF(I)*G0
           GGGF(I)=GGGF(I)*G0
           GGF(I)=GGF(I)+(ST*FQE(I)**0.75)*WB
           GGGF(I)=GGGF(I)+(OS*FQE(I)**0.75+FQE(I)*Vs)*WB
        END DO
        WRITE(24,*) 'BENDING MODES INCLUDED'
        WRITE(*,*) 'BENDING MODES INCLUDED'
     END IF

     WRITE(24,*) JJ,II,'LOOP FINISHED'
     WRITE(24,*) 'REM',REM
     WRITE(*,*) JJ,II,'LOOP FINISHED'
     WRITE(*,*) 'REM',REM

     !==================================================================
     !		 FINDING SIMULATED LOCAL RHEOLOGICAL FEATURES
     !==================================================================

     FQFC1=10**(FMI+DW)	         ! SIMULATED 1st CROSSOVER FREQUENCY 
     FQFM=10**(FMI+DW*NUMOU)	 ! SIMULATED INTERMEDIATE FREQUENCY 
     GGFC1=GG(1)		 ! SIMULATED VALUE OF G' AT 1st CROSSOVER  
     GGGFC1=GGG(1)		 ! SIMULATED VALUE OF G" AT 1st CROSSOVER
     GGFM=GG(NUMOU)		 ! SIMULATED VALUE OF G' AT INTERMEDIATE FREQUENCY 
     GGGFM=GGG(NUMOU)	    	 ! SIMULATED VALUE OF G" AT INTERMEDIATE FREQUENCY
	...
	WRITE(*,*) '*VERROR:',VERR	
     ...
     !   IF (FLAGPR.EQ.1) THEN
     OPEN(10,FILE='RESULT_0.DAT')	! OUTPUT FILE WITH MINIMUM FITTING ERROR
     ...
     IF (FLAGD.EQ.1) THEN
	WRITE(*,*) JJ,II,' ITERATION SUCCEEDED'
	WRITE(24,*) JJ,II,' ITERATION SUCCEEDED'
...
SUBROUTINE MPDR(CLISTi1,LMAPi1,CLISTo1)
  ...
  ITERs1=20
  DO Is1=1,2*INT(CLISTi1(1))+1 ! THE POSITION OF POINTERS 
  ...
  S03:DO Is1=1,INT(CLISTo1(1))		  ! ANNIHILATION OF POINTERS
...
SUBROUTINE GGT(INDi4)

  USE SHARED

  INTEGER :: Is4,Js4,Ks4,IIs4
  INTEGER, INTENT(IN)	:: INDi4

  Is4=1
  OPEN(22,FILE='TEMP.DAT')

  S05:DO Ks4=1,1500			! PICK APPROPRIATE NUMBER OF THE DATA FROM TEMPORARY FILE 

     IF (Is4.LE.50) THEN
        READ(22,*,END=01) Js4,T(Is4),G(Is4)
        IF (Js4.LT.0) THEN
           GOTO 01
        END IF
        Is4=Is4+1
     ...
     IF ((Is4.GT.50).AND.(Is4.LE.100)) THEN
        DO IIs4=1,INT(INDi4-1.)
           READ(22,*,END=01) Js4
           IF (Js4.LT.0) THEN
              GOTO 01
           END IF
     ...
     IF ((Is4.GT.100).AND.(Is4.LE.200)) THEN
        DO IIs4=1,INT(INDi4**2.-1.)
           READ(22,*,END=01) Js4
           IF (Js4.LT.0) THEN
              GOTO 01
           END IF
     ...
     IF ((Is4.GT.200).AND.(Is4.LE.400)) THEN
        DO IIs4=1,INT(INDi4**3.-1.)
           READ(22,*,END=01) Js4
           IF (Js4.LT.0) THEN
              GOTO 01
           END IF
...
SUBROUTINE GEAM(FLAGi5,Ao5,Bo5,To5,ERRMo5)
  ...
  REAL :: RNDs5,A(60,40),IND1s5,IND2s5,TAO(60,40),OPT1,OPT2
  REAL :: RSUM1s5,ERR(60),MINs5,XCHCs5,R1s5,RSUM2s5,R2s5
  REAL, INTENT(OUT) :: Ao5(40),To5(40),ERRMo5
  INTEGER :: Is5,Js5,Ks5,IIs5,RESCALE1,RESCALE2,CHECK,POINT,JJs5,KKs5
  INTEGER :: XCHs5,Ms5,PT1,PT2,B(60,40),Ds5
  ...
  DO Is5=1,SAMPLE			  ! GENERATE RANDOM ENSEMBLE FOR GA FITTING PARAMETERS
     DO Js5=1,NN      
        ...
        A(Is5,Js5)=RNDs5*9.+1.
        CALL RANDOM_NUMBER(RNDs5)
        B(Is5,Js5)=FLOOR((RNDs5-1.)*5.)
        IF ((Js5.EQ.NN).AND.(Js5.EQ.1)) THEN
           B(Is5,Js5)=-3
        END IF
  ...
  S06:DO Ks5=1,GENERATION 	! EVOLUTION OF THE FITTING ENSEMBLE 
     ...
     S07:DO Is5=1,SAMPLE			! NORMALIZE THE FITTING ENSEMBLE 
        ...
        S12:DO WHILE ((CHECK+RESCALE1+RESCALE2).NE.0)
           ...
           B(Is5,1)=MAX(B(Is5,1),-5)
           B(Is5,1)=MIN(B(Is5,1),-1)
           B(Is5,NN)=MAX(B(Is5,NN),-5)
           ...
           DO Js5=1,NN-POINT
              ...
              IF (ABS(Ds5).GT.1) THEN
                 ...
                 B(Is5,NN-Js5)=MAX(B(Is5,NN-Js5),-5)
           ...
           END DO
           IF (ABS(RSUM1s5-1.).LT.10.**(-4.)) THEN
              GOTO 02
           ELSE
              RESCALE2=1
           END IF
           DO Js5=1,NN
              A(Is5,Js5)=A(Is5,Js5)/RSUM1s5
...
02         RESCALE1=0 
           ...
           DO Js5=1,NN
              IF (T(NUMG)/TAO(Is5,Js5).LT.10.) THEN
                 ...
                 RSUM2s5=RSUM2s5+R1s5
              END IF
           END DO

           S13:IF (ABS(RSUM1s5-G(NUMG)).LT.10.**(-5.)) THEN
              GOTO 03	
           ELSE
              S14:IF (RSUM1s5.GT.G(NUMG)) THEN
                 R1s5=RSUM1s5/G(NUMG)
                 R2s5=(1.-RSUM2s5/R1s5)/(1.-RSUM2s5)	
                 DO Js5=1,NN
                    IF (T(NUMG)/TAO(Is5,Js5).LT.10.) THEN	
              ...
              ELSE
                 ...
                 RSUM2s5=RSUM2s5+A(Is5,NN-1)*10.**B(Is5,NN-1)/EXP(T(NUMG)/TAO(Is5,NN-1))
                 R1s5=(G(NUMG)-RSUM1s5)/RSUM2s5+1.
                 R2s5=1.-R1s5*A(Is5,NN)*10.**B(Is5,NN)/EXP(T(NUMG)/TAO(Is5,NN))
                 R2s5=R2s5-R1s5*A(Is5,NN-1)*10.**B(Is5,NN-1)/EXP(T(NUMG)/TAO(Is5,NN-1))
                 R2s5=(1.-RSUM2s5)/R2s5
...
03         RESCALE2=0
     ...
     S08:DO Is5=1,SAMPLE		! CALCULATE THE FITNESS 
        ...
        DO Js5=1,NUMG
           ...
           DO IIs5=1,NN
              ...
              END IF
           END DO
           IF (G(Js5).GT.0.3) THEN
              ERR(Is5)=ERR(Is5)+ABS(G(Js5)-GF(Js5))
           ELSE
              ERR(Is5)=ERR(Is5)+1.25*ABS(G(Js5)-GF(Js5))/G(Js5)
           END IF
     ...
     DO Is5=(3*SAMPLE/4+1-NELT),(3*SAMPLE/4)	! KEEP THE ELITES
        DO Js5=1,NN
     ...
     S09:DO Is5=1,(3*SAMPLE/8-NELT/2) 				! CROSS-OVER 
        CALL RANDOM_NUMBER(RNDs5)
        IF (RNDs5.LE.PC) THEN
     ...
     S10:DO Is5=1,(3*SAMPLE/4-NELT)				  ! MUTATION
        DO Js5=1,NN
     ...
     DO Is5=(3*SAMPLE/4)+1,SAMPLE		! SELECTION 
        DO Js5=1,NN
           CALL RANDOM_NUMBER(RNDs5)
           A(Is5,Js5)=RNDs5*9.+1.
           CALL RANDOM_NUMBER(RNDs5)
...
SUBROUTINE CFER(WLi6,WMi6,WHi6,FERo6,PERo6)
  ...
  FERo6=0.			! THE AVERAGE ERROR OF DIFFERENT REGIONS
  ...
  PERo6(1)=PERo6(1)/2.	        ! MAGNITUDE DIFFERENCE OF 1st CROSSOVER POINT 
  ...
  PERo6(2)=LOG(FQFC1/FQC1)      ! FREQUENCY DIFFERENCE OF 1st CROSSOVER POINT
  ...
  PERo6(3)=LOG(GGGFM/GGGM)	! MAGNITUDE DIFFERENCE OF INTERMEDIATE POINT
  ...
  PERo6(4)=LOG(FQFM/FQM)	! FREQUENCY DIFFERENCE OF INTERMEDIATE POINT
