...
PROGRAM MAIN
  ...
  INTEGER :: LROUSE,LBEND,NUMOU,OUTF,OUTW,OUTU,IDATE(8),NLFU,NLFL,JMAX,JFMAX
  ...
  APHMA=50.		       ! THE UPPER BOUNDARY OF Le/Lp
  APHMI=0.1		       ! THE LOWER BOUNDARY OF Le/Lp
  RATIOMA=2000.		       ! THE UPPER BOUNDARY OF BREAKAGE RATE (ZETA)
  RATIOMI=1E-4                 ! THE LOWER BOUNDARY OF BREAKAGE RATE (ZETA)
  LpMA=2.E-7      	       ! THE UPPER BOUNDARY OF Lp
  LpMI=1.5E-8     	       ! THE LOWER BOUNDARY OF Lp
  ZEMA=500.		       ! THE UPPER BOUNDARY OF Z
  ...
  OPEN(16,FILE='INPUT_v3_3.DAT')	! EXPERIMENTAL DATA INPUT FILE
  READ(16,*) TITLE
  READ(16,*) TEM,PHI,Vs
...
10 CLOSE(16)
  ...
  IF ((FLAGF.EQ.0).and.(out_fit.eq.1)) THEN
     WRITE(24,*) 'THE DATA DO NOT EXSIST'
     WRITE(*,*) 'THE DATA DO NOT EXSIST'
     STOP
  END IF
  ...
  IF (FLAGF.EQ.1) THEN
     FMI=MIN(real(FLOOR(LOG10(FQE(1)))),-2.)           ! THE MINIMUM FREQUENCY OF EXPERIMENTAL DATA 
     FMA=MAX(real(CEILING(LOG10(FQE(NUMIN)))),5.)      ! THE MAXIMUM FREQUENCY OF EXPERIMENTAL DATA
  ...
  IF ((FLAGIM.EQ.0).and.(out_fit.eq.1)) THEN
  ...
  G0MA=GGM*10. 		! THE UPPER BOUNDARY OF PLATEAU MODULUS
  ...
  Z00:DO WHILE (JJ.LE.ITER)	! ITERATION STARTS				 			
	...
	FLAGN=1
     ...
     IF ((FLAGN.EQ.1).or.(out_fit.eq.0)) THEN
	NUM=NUM1
     ELSE 
	NUM=NUM1/2
     END IF
...
21   WRITE(24,*) 'LINEAR LENGTH DISCRETIZATION FINISHED'
     WRITE(*,*) 'LINEAR LENGTH DISCRETIZATION FINISHED'
	...
	END DO
     ...
     !====================================================================================
     !   LOW FREQUENCY SIMULATION FOR ENTANGLED MICELLES (REPTATION, BREAKAGE+REJOINING)
     !====================================================================================
...
31            CALL RANDOM_NUMBER(RND)
              ...
              IF (MOD(K,INV).EQ.0) THEN
                 WRITE(24,*) 'CHAIN BREAKAGE OCCURS:',K
                 WRITE(*,*) 'CHAIN BREAKAGE OCCURS:',K
              END IF
              NLF=NLF+1
...
33            CALL RANDOM_NUMBER(RND)
              ...
              IF (MOD(K,INV).EQ.0) THEN
                 WRITE(24,*) 'CHAIN RECOMBINATION OCCURS:',K
                 WRITE(*,*) 'CHAIN RECOMBINATION OCCURS:',K
              END IF
              NLF=NLF-1
	...
	END IF
     ...
     if (mode.eq.1) then
        ...
        TAGET_TAO(1)=1./TAGET_TAO(1)
        TAGET_MIU(1)=(Y+TAGET_TAO(1)*X)/real(NUMG)
        write(*,*) 'tao:',TAGET_TAO(1)
        write(*,*) 'weight:',exp(TAGET_MIU(1))
        errg=0.
        do I=1,NUMG
           errg=errg+abs(G(I)-exp(TAGET_MIU(1))*exp(-T(I)/TAGET_TAO(1)))/G(I)
           GF(I)=exp(TAGET_MIU(1))*exp(-T(I)/TAGET_TAO(1))
        end do
        errg=errg/real(NUMG)
        write(*,*) 'error:',errg
	...
	OPEN(28,FILE='rotary.dat')  ! WRITE OUTPUT FILE FOR CONTRIBUTION TO G'&G" FROM ROTARY MODES FOR UNENTANGLED MICELLES
 write(28,*) 'clenth',M,N*Ls*TL,3.*d/Lp,APH
	...
	END DO
     ...
     IF (OUT_sep.EQ.1) THEN
	OPEN(29,FILE='rouse_u.dat')  ! WRITE OUTPUT FILE FOR CONTRIBUTION TO G'&G" FROM ROUSE MODES FOR UNETANGLED MICELLES
	...
	END DO
     ...
     IF (OUT_sep.EQ.1) THEN
	OPEN(30,FILE='bend_u.dat')  ! WRITE OUTPUT FILE FOR CONTRIBUTION TO G'&G" FROM BENDING MODES FOR UNETANGLED MICELLES
	...
	WRITE(9,*) 'MICELLE LENGTH',LBAR*TL 	
     ...
     !=======================================================================
     !	 HIGH FREQUENCY SIMULATION (ROUSE+BENDING MODES, ENTANGLED MICELLES)
     !=======================================================================
        ...
        B02:DO I=1,NUMOU		   ! ROUSE MODES FOR ENTANGLED MICELLES
           ...
           DO J=1,M
              ...
              IF (CLENTH.GE.APH) THEN
                 ...
                 GGC=0.
                 GGGC=0.
                 if (ze.le.15) then ! Full  Rouse modes
                    DO K=1,NM
                       GGC=GGC+(W*tR/2/K**2.)**2./(1+(W*tR/2/K**2.)**2.)
                       GGGC=GGGC+W*tR/2/K**2./(1+(W*tR/2/K**2.)**2.)
                    END DO
                 else if (ze.gt.15) then ! Fractionated Rouse modes
                    DO K=1,Z-1
                       GGC=GGC+(W*tR/K**2.)**2./(4.*(1+(W*tR/K**2.)**2.))
                       GGGC=GGGC+W*tR/K**2./(4.*(1+(W*tR/K**2.)**2.))
                    END DO
                    DO K=Z,NM
                       GGC=GGC+5.*(W*tR/2/K**2.)**2./(4.*(1+(W*tR/2/K**2.)**2.))
                       GGGC=GGGC+5.*W*tR/2/K**2./(4.*(1+(W*tR/2/K**2.)**2.))
                    END DO
                 end if
                 GGS=GGS+GGC*G0*VOF/Z
                 GGGS=GGGS+GGGC*G0*VOF/Z
              END IF
           END DO
           ...
           DO J=1,M
              ...
              IF (CLENTH.GE.APH) THEN
                 ...
                 if (ze.le.15) then ! Full Rouse modes
                    ...
                    DO K=1,Z-1
                       GGC=GGC+(FQE(I)*tR/K**2.)**2./(4.*(1+(FQE(I)*tR/K**2.)**2.))
                       GGGC=GGGC+FQE(I)*tR/K**2./(4.*(1+(FQE(I)*tR/K**2.)**2.))
                    END DO
                    DO K=Z,NM
                       GGC=GGC+5.*(FQE(I)*tR/2/K**2.)**2./(4.*(1+(FQE(I)*tR/2/K**2.)**2.))
                       GGGC=GGGC+5.*FQE(I)*tR/2/K**2./(4.*(1+(FQE(I)*tR/2/K**2.)**2.))
                    END DO
                 end if
                 GGS=GGS+GGC*G0*VOF/Z
                 GGGS=GGGS+GGGC*G0*VOF/Z
	...
	WRITE(24,*) 'BENDING MODES INCLUDED'
        ...
        if (out_sep.eq.1) then
           OPEN(32,FILE='bending.dat')  ! output file for bending modes
	...
	END DO
     ...
     IF (FLAGMGT.EQ.0) THEN
        DGFMAX=(GG(1)-GGG(1))/GGG(1)
        JFMAX=1
        DO J=1,NUMOU
           IF ((GG(J)-GGG(J))/GGG(J).GT.DGFMAX) THEN
              DGFMAX=(GG(J)-GGG(J))/GGG(J)
              JFMAX=J
           END IF
        END DO
	FQFM=10**(FMI+DW*JFMAX)
     ELSE
	...
	END IF
     ...
     if ((out_fit.eq.1).or.((out_fit.eq.0).and.(flagf.eq.1))) then
	...
	WRITE(10,*) 10**(FMI+DW*I),GG(I),GGG(I)
     ...
     ! Uncomment this section to write fitted crossover frequencies/moduli
     ! + G', G" at experimental input points to a file
     ! Can be useful to double check points/error calculation
     if ((flagpr.eq.1).or.(flagd.eq.1)) then
        open(33,file='result_in.dat')
        write(33,*) 'Experimental features:'
        write(33,*) '1st crossover frequency',FQC1
        write(33,*) 'G at 1st crossover',GGC1,GGGC1
        write(33,*) 'Intermediate frequency',FQM
        write(33,*) 'G at intermediate frequency',GGM,GGGM
        write(33,*) '2nd crossover frequency',FQC2
        write(33,*)
        write(33,*) 'Simulation features:'
        write(33,*) '1st crossover frequency',FQFC1
        write(33,*) 'G at 1st crossover',GGFC1,GGGFC1
        write(33,*) 'Intermediate frequency',FQFM
        write(33,*) 'G at intermediate frequency',GGFM,GGGFM
        write(33,*) '2nd crossover frequency',FQFC2
        write(33,*)
        !do i=1,numin
        !   write(33,*) FQE(I),GGF(I),GGGF(I)
        !end do
        close(33)
     end if
...
42 IF ((FLAGD.EQ.1).OR.(JJ.EQ.ITER).or.(out_fit.eq.0)) THEN
   ...
   if ((JJ.eq.iter).and.(flagd.eq.0)) then
...
SUBROUTINE GGT(Ni4,G1i4,G2i4)
  ...
  IF (FLAGs4.EQ.0) THEN			 ! WRITING TIME DOMAIN DATA IN THE TEMPORARY FILE
...
02         CALL RANDOM_NUMBER(RNDs5)
  ...
  ERRSD=1000.
  S07:DO Ks5=1,GENERATION 	! EVOLUTION OF THE FITTING ENSEMBLE 
...
SUBROUTINE CFER(WLi6,WMi6,WHi6,FERo6,PERo6)
  ...
  PERo6(3)=LOG(GGGFM/GGGM*GGM/GGFM)	! MAGNITUDE DIFFERENCE OF INTERMEDIATE POINT
